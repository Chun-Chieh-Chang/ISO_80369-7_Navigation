...
### 需求來源
使用者反饋與目標受眾鎖定：本簡報主要展示場景為「內部研發/QC 工程師的技術 Training」（追求極致乾貨、零廢話）。使用者認為原第 11、12 頁充斥著沒有營養的感性公關口號，指示執行「文案脫油與高密度重構」。

### 1. 診斷與根因 (RCA)
- **初始文案偏誤**：原第 11 頁以「生命防護網」、「心裡踏實」、「走出國門」等感性宣傳文案為主，第 12 頁以「致敬無名英雄」、「極致溫柔」等抒情口號收尾。對於「研發/QC 內部技術培訓」受眾而言，資訊密度過低且缺乏工程落地指引，產生嚴重的「AI 塑料感與罐頭公關套話」。
- **改善方針**：全面剔除所有抒情與宣傳口號，將 Slide 11 與 Slide 12 改造為純粹面向工程實務的硬核培訓內容：
  - **Slide 11**：定位為「工程全鏈路・R&D / QC / RA 協同實務」，涵蓋 4 階段流程（開模尺寸公差定錨 -> 治具與手法極限驗證 -> 技術文件交付 -> 量產 Cpk 管制）與 3 大核心工程角色責任矩陣。
  - **Slide 12**：定位為「培訓精華・技術第一性原理」，總結 ISO 80369 體系三大不可動搖的底層工程法則（三維幾何物理防呆、母標準裁判與工具箱解耦、固體與流體力學真實機制）。

### 2. 矯正與預防措施 (CAPA)
- **文案脫油與資訊密度提升**：
  - `public/slides/index.html` Slide 11 與 Slide 12 內容全數以工程參數、LMC/MMC 實體極限、6% 錐度補償、Cpk 管制、Annex C 治具校準等硬核技術語言重構。
  - **參數溯源修正 (SSOT 確效)**：Slide 11 階段 2「QC 實驗室極限驗證」更正參考連接器表面粗糙度筆誤，由 0.4 µm 修正為法定精確值 **Ra ≤ 0.8 µm**（依據 ISO 80369-7:2021 Annex C Figures C.1–C.6 之法定標準件要求），與系統 SSOT 資料庫 (`isoTopicsData.ts`、`ConnectorInspector.tsx`、Slide 8) 保持 100% 一致。
  - **方案 B「標準法定 vs 工廠實務」嚴格標註分流確效**：
    - Slide 11 全面採用標籤解耦：明確標記 `[ISO 法定]`（如 6% 錐度基準 1:16.667、Annex C 治具 Ra ≤ 0.8 µm、雙序列預裝配 26.5~27.5 N + 0.08~0.12 N·m、Clause 5 三維 CAD 干涉報告）vs `[模具實務]`（樹脂收縮率預補償）vs `[品保實務]`（統計製程 SPC 管制圖建議 Cpk ≥ 1.33）vs `[儀校實務]`（治具定期磨損檢定）vs `[體系管理]`（ISO 13485 ECO 變更管制）。
    - 幾何名詞修正：更正 Slide 1 與 Slide 5 將 Ø3.97mm 誤稱為基底外徑之處，精準標記為「公接頭尖端外徑 (Tip Minor Diameter Ød 3.970~4.035 mm)」，並註明凱益實物 I.D. 為軟管倒鉤型錄孔徑。
    - Slide 4 跨領域接口對比優化：明確區分臨床直觀辨識特徵（ENFit 粗大反牙、NRFit 細徑帶環）與 ISO 80369-3/6 專屬工程圖紙定錨。
  - 同步重寫兩頁之演講者備忘錄 (`aside.notes-data`)，提供講者專業內訓口播話術。
- **響應式視窗自適應優化**：
  - 微調 Slide 11 卡片內部間距與字級行高，確保在筆電或小螢幕（`max-height: 760px`）環境下完全自適應貼合，底端核心交付物無任何截斷溢出。
- **離線單檔同步編譯**：
  - 執行 `npm run build:standalone`，產出同步更新後的 `public/slides-standalone.html`（27.19 MB）。
- **版本號同步升級**：`package.json` 由 8.42.1 升至 8.43.0。

### 3. 驗收標準 (Acceptance Criteria)
- [x] `npm run lint`（tsc --noEmit）— 0 errors
- [x] `npm run test`（vitest run）— 52/52 PASS
- [x] `npm run build`（vite build）— PASS
- [x] `npm run build:standalone` — 成功產出 standalone 單檔
- [x] 瀏覽器實測截圖驗證：Slide 11 與 Slide 12 視覺結構完美、無溢出截斷、Overview 縮圖標題自動同步、演講者抽屜話術切換正常、Console 零錯誤。

---
## 版本：v8.42.1 (Commit: 6d9aaef) 規範附件圖表導航樹所有抽屜預設收合狀態優化 (2026-09-19)
...
- **使用者於 2026-09-07 18:07 人工視覺確認：渲染正確、訊息完整、高保真無損。PDCA 閉環完成。**
...
依據 ISO 80369-7:2021 Clause 6.6 明文規範：
- 公鎖定接頭 (Internal Luer lock) 測試必須採用 **Figure C.3**（具備 2.71 mm 窄突耳之最壞情況母件）；
- 母鎖定接頭 (External Luer lock) 測試必須採用 **Figure C.6**（具備最壞情況公螺紋之金屬夾具）。

### 根因分析 (RCA)
- *現象 1*：`STANDARD_CLAUSE_DETAILS['iso7-6.6'].fixtureRequiredZh` 僅撰寫了公件單向之 Fig.C.3，造成 `ClauseComparisonMatrix.tsx` 讀取 SSOT 時僅渲染出 Fig.C.3。
- *現象 2*：過載滑牙測試描述早期過度聚焦於公端套環環向張應力膨脹 (Hoop Expansion)，忽略母端突耳在最壞情況淺牙公螺紋 (Fig.C.6) 作用下之剪切破壞考驗。

### 矯正與預防措施 (CAPA)
1. **SSOT 資料庫修訂 (`src/data/isoTopicsData.ts`)**：
   - 擴展 `iso7-6.6` 與 `iso20-annex-h` 的 `fixtureRequiredZh` 為：`Fig.C.3 母參考接頭（公件受測：2.71 mm 窄耳翼） / Fig.C.6 公參考接頭（母件受測：最壞情況淺牙螺紋）`。
   - 更新適用對象為「所有魯爾鎖定接頭（公鎖定 Male Lock 與母鎖定 Female Lock）」。
   - 更新試驗步驟與卡片 tags/標籤，公母最壞情況夾具全面並列。
2. **對照矩陣元件全面對齊 (`src/components/ClauseComparisonMatrix.tsx`)**：
   - 更新 Clause 6.6 與 Annex C 橫向對照列，不論中英文環境均 100% 完整顯示 Fig.C.3 與 Fig.C.6。
3. **物理核心與多語系同步 (`src/data/isoData.ts`, `src/utils/i18nHelpers.ts`, `src/i18n/translations.ts`)**：
   - `keyPhysicsZh`、`fixtureRequiredEn` 與 DVP 審查注意事項同步修正為公配 Fig.C.3、母配 Fig.C.6。
4. **確效驗證**：
   - 啟動 `browser_subagent` 實際進入對照矩陣，擷取畫面確效驗證，6.6 橫向對照列 100% 正確顯示 `Fig.C.3 母參考接頭（公件受測：2.71 mm 窄耳翼） / Fig.C.6 公參考接頭（母件受測：最壞情況淺牙螺紋）`。
   - `npm test`: 17/17 PASS
   - `npm run lint`: 零錯誤 PASS
   - `npm run build`: 5.54s 打包成功 PASS

---
## 版本：v8.40.0 (Commit: 1fd8843) 國際標準 SSOT 溯源：全面修復金屬參考夾具 (Annex C) 圖面映射中斷、公差參數與投影片演示對齊 (2026-09-07)

### 需求來源與目標
基於 SSOT 原則，調用 `isodoc/ISO_80369-7_2021_en.pdf` (Annex C、Clause 4.1、Clause 6.1~6.6) 與 `isodoc/ISO_80369-20_2024_en.pdf` 原始文檔，對「參考金屬夾具庫」及投影片演示進行全面深度審查與閉環修復：
1. **修復圖資映射中斷**：解決前端在點選 C.3、C.4、C.5、C.6 及 B.1、B.2 時，錯誤顯示「本主題無 ISO 80369-7 幾何尺寸藍圖」的失聯問題。
2. **校正法規參數與術語偏差**：
   - 修正 C.4 誤寫為「外螺紋」之術語瑕疵，正名為「套環內螺紋 (Collar Internal Thread)」。
   - 補充 C.6 最壞情況關鍵極限參數（Ø8.0 mm 槽底大徑、Ø7.2 mm 牙頂小徑、30° 牙面角）。
   - 補充 C.2 與 C.5 錐長/孔深之雙重法定標準（一般性能測試 ≥ 7.5 mm；Clause 4.1 防誤接試驗 ≥ 10.5 mm）。
   - 補充 C.4 與 C.5 Note 2 警示條款（套環外徑不得用於防誤接檢驗）。
   - 精確分流 Annex C.1 法定材質文字（耐腐蝕剛性材料，E > 3,433 MPa，Ra ≤ 0.8 µm）與工程實務（硬化不鏽鋼抗磨損與抗咬死）。
3. **投影片演示 (Slide Deck) 糾錯**：
   - 修正 Slide 7 誤將 C.1 標示為公頭（`C.1/C.2 鋼製公頭` ➔ `C.1 母鎖 / C.2 公滑 / C.4 公鎖 / C.5 母滑 標稱件`）的性別顛倒問題。
   - 修正 Slide 7 將 C.3 標記為 `±0.03 mm` 之非標準公差，改為法定標準 `2.71 (+0.025/0 mm)`。
   - 修正 Slide 6 中將 Annex C 標準夾具誤歸屬於 ISO 80369-20 的標準拼貼問題（落實 Part 7 規格與 Part 20 手法之嚴格解耦）。
   - 補齊 Slide 9 中 Clause 6.6 考驗條件之公母雙向配對（公件配 C.3 窄耳翼 / 母件配 C.6 極限淺牙）。
   - 重新編譯產出單檔獨立投影片 `public/slides-standalone.html`（27.19 MB）。

### 根因分析 (RCA)
- *現象 1（圖面映射遺漏）*：
  - `ISOStandardFigureRenderer.tsx` 內 `getBlueprintImagePath` 在過往重構中，Switch 分支僅保留 `ISO7-FIG-C1` 與 `ISO7-FIG-C2`，未將已存在於 `public/assets/blueprint/` 的 `page_11.png` (C.3)、`page_12.png` (C.4)、`page_13.png` (C.5)、`page_14.png` (C.6)、`page_1.png` (B.1) 與 `page_2.png` (B.2) 進行映射，導致 UI 點擊後 fallback 到無圖面警告。
- *現象 2（公鎖定套環內螺紋混淆）*：
  - Luer Lock 系統中，公端接頭外部設有螺紋套環（Threaded Collar），其螺紋位於套環內部（內螺紋），母端接頭之突耳（Lugs）或外螺紋轉入公套環內部咬合。文字描述寫成「外螺紋」產生嚴重技術混淆。
- *現象 3（投影片文字顛倒與舊公差殘留）*：
  - Slide 7 快速撰寫時將 C.1 與 C.2 合併簡稱為「鋼製公頭」，忽略 C.1 實為 Female Luer Lock（母座）；且 C.3 耳翼標註了早期草稿的 `±0.03 mm`，非 ISO 80369-7:2021 正式發布之 `2.71 (+0.025/0 mm)`。

### 矯正與預防措施 (CAPA)
1. **完善藍圖圖片映射 (`src/components/ISOStandardFigureRenderer.tsx`)**：
   - 完整補齊 `ISO7-FIG-B1` (`page_1.png`)、`ISO7-FIG-B2` (`page_2.png`)、`ISO7-FIG-C3` (`page_11.png`)、`ISO7-FIG-C4` (`page_12.png`)、`ISO7-FIG-C5` (`page_13.png`)、`ISO7-FIG-C6` (`page_14.png`) 映射分支。
2. **校正資料庫 (`src/data/isoData.ts`)**：
   - 修正 C.4 為「套環內螺紋 (Collar Internal Thread)」，消除外螺紋誤稱。
   - 補充 C.6 關鍵極限尺寸（Ø8.0 mm 槽底大徑、Ø7.2 mm 牙頂小徑、30° 牙面角）。
   - 補充 C.2 與 C.5 錐長雙標準（性能 ≥ 7.5 mm / 防誤接 ≥ 10.5 mm）。
   - C.1~C.6 補齊精準公差（C.1 3.50 0/-0.025 mm, C.3 2.71 +0.025/0 mm）。
3. **完善標準指導卡片 (`src/components/ConnectorInspector.tsx`)**：
   - 明確分流 ISO 法定材質要求（耐腐蝕剛性材料，E > 3,433 MPa）與工程實務（硬化不鏽鋼抗磨損）。
   - 增加 Note 2 套環外徑不得用於非相互連接檢驗之重要法規提示。
4. **校正投影片演示 (`public/slides/index.html` & `public/slides-standalone.html`)**：
   - Slide 6：解耦 Part 7 規格與 Part 20 手法。
   - Slide 7：修復 C.1 性別倒置問題，校正 C.3 公差為 2.710~2.735 mm，補齊標稱件 (C.1/C.2/C.4/C.5) 與最壞情況件 (C.3/C.6)。
   - Slide 9：補齊 6.6 考驗條件公母雙向配對（C.3 / C.6）。
   - 重新編譯 `public/slides-standalone.html`（27.19 MB）。
5. **確效驗證與基準點建立**：
   - `npm test` (vitest): 17/17 PASS
   - `npm run lint` (tsc): 零錯誤 PASS
   - `npm run build` (vite build): 5.66s 打包通過 PASS
   - Git 還原基準點 Commit: `1fd8843`

---
## 版本：v8.39.0 國際標準 SSOT 溯源矯正：全面根除 ISO 80369-7 §6.3 與 ISO 80369-20 Annex E 應力龜裂 4 大矛盾與法規引用錯誤 (2026-09-05)

### 需求來源與目標
使用者深度比對系統中掛載 ISO 80369-7:2021 §6.3 與 ISO 80369-20:2024 Annex E 的兩張系統卡片，基於第一性原理提出權威審查意見，指出編寫者在理解「產品規範（Part 7）」與「測試方法（Part 20）」時出現認知混亂、文字拼貼與 4 大核心矛盾與法規錯誤：
1. **致命錯誤**：圖一允許用「6.2 負壓測試」驗收，嚴重違反標準（標準明訂限用 6.1.1 正壓）。
2. **溫度矛盾**：環境溫度 20°C–30°C vs 15°C–30°C，且圖一步驟 2 與步驟 3 前後打架。
3. **名稱誤植**：圖一多加了「耐環境 (Environmental)」，混淆了乾燥空氣組裝應力龜裂與化學溶劑 ESCR (如 ASTM D1693) 之本質。
4. **允收標準迷思**：將「目視判定」列為唯一法定判定，忽略法規唯認「6.1.1 洩漏測試 Pass」。

### 根因分析 (RCA)
- *現象 1（允收標準違反標準）*：
  - ISO 80369-7:2021 第 6.3 節明文規定：“Luer connectors shall meet the requirements of 6.1.1 after being subjected to stresses of ISO 80369-20:2015, Annex E.”。
  - 力學原理：負壓（真空抽吸）會將塑料向內吸合（微裂紋閉合），難以有效檢測裂紋；唯有施加 300~330 kPa 正壓流體，才會將微小裂紋撐開導致洩漏。舊版卡片文字拼貼加入「或 6.2 負壓空氣洩漏」屬於嚴重法規瑕疵。
- *現象 2（溫度規格混亂）*：
  - ISO 80369-20:2024 Annex E.2.2 規定測試環境溫度為 15 °C 至 30 °C（10% 至 70% RH），承襲自 ISO 594 傳統常溫寬公差。
  - 圖一上方標註 20°C–30°C，步驟 2 寫 23°C，步驟 3 又寫 20–30°C，為多份舊草稿剪貼殘留的內部矛盾。
  - 工程實務上，醫材實驗室依 ISO 291 採用標準環境 23 ± 2 °C、50% RH，完全涵蓋標準之 15~30 °C。
- *現象 3（標題誤植「耐環境 Environmental」）*：
  - ISO 80369-7 §6.3 正式標準英文名稱為純粹的 “Stress cracking”，絕無 “Environmental”。
  - ESCR（如 ASTM D1693）係評估高分子接觸界面活性劑、化學藥劑或醇類之開裂；而 ISO 80369-7 §6.3 僅評估塑料組裝於金屬夾具後在常溫常濕空氣中靜置 48 小時承受純機械預緊環向應力（Hoop Stress）的抗潛變能力，無任何外加化學介質。
- *現象 4（目視判定主客顛倒）*：
  - 法規唯一的法定裁決標準為通過 Clause 6.1.1 正壓流體洩漏測試。微裂紋（Crazing）常小於微米級，目視無法作為法定唯一標準，在標準 Annex E.5(h) 中僅作為實驗室工程觀察記錄。

### 矯正與預防措施 (CAPA)
1. **全面矯正 `src/data/isoTopicsData.ts`**：
   - 修正 `iso7-6.3` 標題為「6.3 應力龜裂規範條文 (Stress Cracking Requirement)」。
   - 修正客觀目的，明確界定「維持預裝配環向過盈應力下，於乾燥空氣中靜置 48 小時之抗潛變開裂能力」。
   - 統一環境溫度為 `15°C - 30°C (建議 23 ± 2°C)`，並刪除重複的步驟 2/3。
   - 步驟 3（原步驟 4）徹底刪除「或 6.2 負壓」，強制規定「進行 6.1.1 正壓流體洩漏測試（氣壓衰減法 Annex B 或正壓液體落滴法 Annex C）驗證密封性」。
   - 允收標準分離為「法規法定判定標準 (Statutory Acceptance Criteria)」與「實驗室工程紀錄 (Engineering Record)」。
   - 修正 `iso20-annex-e` 與 `topic-stress-cracking`，全面清除「耐環境」與「化學介質引發」等非標準腦補文字。
   - **多維度審查新成果**：修正 Clause 6.4 軸向拉力主題與圖形，補齊「滑動型 (Slip 23~25 N)」與「鎖定型 (Lock 32~35 N)」分流，更新標籤與 keyCallouts。
2. **同步更新資料庫與矩陣 `src/data/isoData.ts` & `src/components/ClauseComparisonMatrix.tsx`**：
   - **修正 Clause 6.1 歷史留存瑕疵**：將舊版殘留之「保持 10 秒以上」修正為標準持壓時間（氣壓衰減 6.1.2 保持 15~20 秒，洩漏率 ≤ 0.005 Pa·m³/s；液體落滴 6.1.3 保持 30~35 秒，無形成滴落水滴）。
   - 更新 Clause 6.3 的 `passCriteria`、`passCriteriaZh`、`keyPhysics`、`keyPhysicsZh` 與 `ISO20-E.1` 結構。
   - 將條文對照矩陣標題更新為「6.3 應力龜裂 (Stress Cracking)」。
3. **同步更新國際化翻譯字典 `src/utils/i18nHelpers.ts`**：
   - 更新 `stress-cracking`、`axial-separation`、`ISO20-E.1` 及 `getClausePassCriteriaEn` 中對 6.1、6.3、6.4 的英文描述與完整公差區間（300~330 kPa, 32~35 N / 23~25 N, 0.15~0.17 N·m）。
...
### 需求來源與目標
使用者提出兩階段關鍵指示：
1. 「接頭的各種型態需要以MECE方式呈現，截圖中的4個圖片示例只呈現出兩種型態，請深入解析 ISO 80369-7 的內容，並對應要求修訂投影片。」以及「希望本專案投影片可以用來promote Mouldex 的產品，可以利用 Mouldex 網站的內容包括圖片來做演示。」
2. 「推力之外還缺扭矩的陳述」以及「應水平展開檢查，包含檢索介面」。

### 根因分析 (RCA)
- *現象 1（分類不 MECE）*：原 Slide 5 的 4 張卡片全數落在「L2 帶鎖型 (Luer Lock)」範疇，漏掉了 ISO 80369-7 條文 5 (圖 B.1/B.2) 所定義的「L1 滑套型 (Luer Slip)」。
- *現象 2（物理量陳述不對稱與遺漏）*：
  - 投影片 Slide 1 原文僅提及「27 N 推力與 0.1 N·m」，缺少了「扭矩」的名詞標註與標準完整公差區間（26.5~27.5 N 軸向推力 與 0.08~0.12 N·m 旋緊扭矩）。
  - 檢索介面中，`ClauseComparisonMatrix.tsx` 的預裝配條件欄位寫著 `0.08–0.12 N·m + 26.5–27.5 N 推力`，前面的數值單位缺少了「扭矩」名詞標籤；`DvpGenerator.tsx` 與 `excelExporter.ts` 亦有類似的名詞不對稱狀況；`isoTopicsData.ts` 部分條文測試步驟中僅提扭矩漏提軸向推力。
- *本質解構 (SSOT)*：
  - ISO 80369-20 附錄 D/F/G/H 及 Annex H.4 明確規定預裝配條件為：**26.5~27.5 N 軸向推力 (Axial Force)** 與 **0.08~0.12 N·m 旋緊扭矩 (Torque)**，旋轉角度 ≤ 90°，持載 **5~6 秒** 後完全釋放。
  - 身為醫療法規與高精密工程工具，所有涉及預裝配條件之處，必須維持「推力」與「扭矩」的名詞與數值完整對稱，嚴禁只寫數字或遺漏其中一項。

### 矯正與預防措施 (CAPA)
1. **Mouldex 官方產品與圖片無縫整合**：
   - 爬取並解析 Mouldex 官網產品 API 與高清圖檔：
     - **象限 1 (L1 公滑套, 圖 B.1)**：Mouldex D09-410-111-1（I.D. 4.1mm / 醫用 ABS）。
     - **象限 2 (L2 雙耳母座, 圖 B.6)**：Mouldex D10-270-331（I.D. 2.7mm / 廣泛適配留置針）。
     - **象限 3 (L2 固定公鎖, 圖 B.3)**：Mouldex C09-200-211（I.D. 2.0mm / 承載 300 kPa 正壓）。
     - **象限 4 (L2 旋轉公鎖, 圖 B.4)**：Mouldex SA0145（I.D. 2.79mm / 360° 獨立旋轉防軟管扭結）。
2. **Slide 5 三重 Tab 精密圖紙體系**：
   - **Tab 1**：📷 凱益 (Mouldex) 實體接頭 MECE 完整圖鑑（L1 滑套 vs L2 帶鎖 2×2 展台）。
   - **Tab 2**：📐 官方精密圖紙：L1 滑套標準（圖 B.1 公滑套 ＆ 圖 B.2 母滑套）高清渲染。
   - **Tab 3**：📐 官方精密圖紙：L2 帶鎖標準（圖 B.3 公帶鎖 ＆ 圖 B.6 母帶鎖）。
3. **全站水平展開校正預裝配條件 (推力與扭矩)**：
   - **投影片 (`public/slides/index.html`)**：Slide 1、Slide 7、Slide 10 全數精準校準為「26.5~27.5 N 軸向推力與 0.08~0.12 N·m 旋緊扭矩」。
   - **檢索矩陣 (`ClauseComparisonMatrix.tsx`)**：所有條文欄位統一名詞格式為 `Torque: 0.08–0.12 N·m + Axial Force: 26.5–27.5 N, hold 5–6 s then release` / `扭矩: 0.08–0.12 N·m + 軸向推力: 26.5–27.5 N，維持 5–6 秒後釋放`。
   - **DVP 生成器 (`DvpGenerator.tsx`)**：`renderPreAssembly` 清晰標註 `Torque: ...` 與 `Axial Force: ...`。
   - **資料層 (`isoTopicsData.ts`)**：補齊 6.4、6.5、Annex I、Annex E 的推力與扭矩對稱敘述，並於 6.3 補正 `assemblyAxialForceN: '26.5 N - 27.5 N'`。
   - **匯出模組 (`excelExporter.ts`)**：CSV/Excel 匯出字串全面標註扭矩與軸向推力。
4. **單檔離線編譯**：
   - 執行 `npm run build:standalone`，產出包含所有真品圖片與圖紙之 `public/slides-standalone.html`（27.38 MB）。
5. **軟體確效**：
   - `npm test` 17/17 PASS、`npm run lint` 0 錯誤、`npm run build` 通過。

---
## 版本：v8.35.0 視覺純化：全面清除實體卡片與彈窗前綴型號編碼 (2026-09-05)

### 需求來源與目標
使用者截圖反饋 Slide 5 實體卡片標題（如 `CIML7 帶鎖公接頭`、`CFL7 雙耳母接頭`、`ILB7 儀器面板專用母接頭`、`BNP7 軟管一體膠合公接頭`），提出要求：「截圖中的文字前面不要出現前綴編碼」。目標是徹底拿掉這些晦澀的模具/物料編號前綴，回歸純粹、直觀且專業的中文結構名稱。

### 根因分析 (RCA)
- *現象*：卡片標題帶有 `CIML7`、`CFL7`、`ILB7`、`BNP7` 等英數混合前綴。
- *成因*：這些編碼是模具注塑廠商（如 Industrial Spec）的料號前綴縮寫（如 Coaxial Internal Male Luer 等）。對於非注塑開模領域的受眾與臨床研發者而言，屬於冗餘認知負擔，並非 ISO 80369-7 標準條文本身規定的專有名詞。
- *第一性原理*：科普與工程宣講應以清晰的結構語義為核心，去除廠商私有代碼，確保閱讀與簡報體驗乾淨高級。

### 矯正與預防措施 (CAPA)
1. **Slide 5 卡片標題與 Lightbox 參數純化**：
   - `CIML7 帶鎖公接頭（點滴插頭）` ➔ `帶鎖公接頭（點滴插頭）`
   - `CFL7 雙耳母接頭（留置針接口）` ➔ `雙耳母接頭（留置針接口）`
   - `ILB7 儀器面板專用母接頭` ➔ `儀器面板專用母接頭`
   - `BNP7 軟管一體膠合公接頭` ➔ `軟管一體膠合公接頭`
   - 同步修正圖片 `alt` 屬性與 `openLightbox()` 標題字串。
   - 同步清理演講備忘錄（`notes-data`）中口播詞的 `CIML7` / `CFL7`。
2. **Slide 1 標題與 Lightbox 同步純化**：
   - 移除 Lightbox 呼叫字串中的 `- CIML7 Series` 與 `- CFL7 Series`。
3. **單檔同步編譯**：
   - 執行 `npm run build:standalone`，鏡像更新 `public/slides-standalone.html`（27.06 MB）。
4. **全專案文件同步與 SSOT 對齊**：
   - `README.md`：同步升級版本至 `v8.35.0`，修正離線單檔打包指令（優先推薦 `npm run build:standalone`），並全面更新投影片 12 頁教學大綱，精確反映 Slide 1, Slide 6, Slide 10 的最新架構。
   - `presentation-strategy-brief.md`：清除第 65 行殘留的「50N 暴力強插」描述，精準對齊 ISO 80369-7 條文 5 三維 CAD 幾何防呆零互插分析，貫徹零捏造溯源協定。
5. **版本控制、隱私審查與沙盒確效**：
   - `package.json` 版本號升級至 `8.35.0`。
   - `CHANGELOG.md` 建立詳細版本變更。
   - 隱私審查：確認 `.gitignore` 完備排除 `isodoc/` 內部標準原件與本機暫存檔。
   - 執行自動化確效測試（`npm test` 17/17 PASS、`npm run lint` 0 錯誤、`npm run build` PASS）。

---
## 版本：v8.34.0 開發者本質回歸：清除第三方檢驗所偽劃分，確立單一玩家兩本合參閉環 (2026-09-05)

### 需求來源與目標
使用者提出深刻的工程第一性原理反饋：「對於玩家而言，-7 與 -20 都是必須遵循的規範，即使沒有第三方檢驗所的角色，他也是要遵守，納入檢驗所只會增加複雜度、擾亂視聽，請基於客觀事實修正」。目標是全面清除「第三方檢驗所」的干擾設定，回歸單一核心玩家——「醫材研發製造者 (Developer & Manufacturer)」，說透其如何同時以兩本標準完成設計確效 (DVP)。

### 根因分析 (RCA)
- *現象*：上一版嘗試將 -7 分給醫材廠、-20 分給檢驗所，人為製造了標準割裂。
- *客觀事實 (SSOT)*：
  - 醫材製造商自身就是 -7 與 -20 的絕對遵循主體。在廠內設計階段，工程師必須左手看 -7 定尺寸與及格線，右手看 -20 買治具架機台做組裝量測。
  - DVP（設計驗證報告）中，「接受標準」引用 -7，「試驗方法」引用 -20，兩者是同一工程師手中的左右手工具。

### 矯正與預防措施 (CAPA)
1. **Slide 1 重構**：
   - 移除「第三方檢驗所」等干擾角色。
   - 標題升級為：「⚖️ 追問底層邏輯：醫材開發者為什麼必須『兩本合參』？」。
   - 左卡：`📘 ISO 80369-7 ｜ 設計依據 (What)`，任務為開模研發、微米公差與 6 大及格門檻。
   - 右卡：`🔬 ISO 80369-20 ｜ 量測手冊 (How)`，任務為標準治具架設、裝配 SOP 與通用量測手法。
   - 底部橫條精煉為：「**開發者合規閉環：**翻開 -7 定尺寸與合格線 ⟷ 依據 -20 架治具驗證極限，兩本合璧才能完成產品的設計確效 (DVP)！」。
   - 演講備忘錄 (`notes-data`) 同步更新為工程師兩本合參之臨床故事。
2. **Slide 6 呼應**：
   - 舉例三同步調整為「醫療器材工程師的『兩本合參』日常」。
3. **單檔同步**：
   - 執行 `npm run build:standalone`，產出最新 `public/slides-standalone.html`（27.05 MB）。
4. **確效驗證**：
   - `npm test` 17/17 PASS。
   - `npm run lint` 0 錯誤。
   - `npm run build` 通過。
...
---
## 版本：v8.33.0 標準純度審查：全面清除非 -7/-20 之 50 N 參數，回歸條文 5 CAD 幾何防呆 (2026-09-05)

### 需求來源與目標
使用者提出深刻的第一性原理審查質疑：「專案聚焦於 -7 和 -20，50 牛頓這件事情在 -7 或 -20 都沒有出現，有必要出現在投影片中嗎？」。經 SSOT 溯源查證，50 N 出自 ISO 80369-1:2018 附錄 B，在 ISO 80369-7 與 ISO 80369-20 中皆不存在。目標是徹底清除 50 N，純化 Slide 10，將防呆機制回歸 ISO 80369-7 條文 5 之「三維 CAD 幾何空間干涉分析」。

### 根因分析 (RCA)
- *現象*：Slide 10 原將「條文 6.3 耐應力龜裂（48h）」與「ISO 80369-1 附錄 B 50 N 物理對接互斥」硬湊為「大考之三」，混淆了跨標準歸屬。
- *SSOT 真相*：
  - **ISO 80369-7 條文 5 (Non-interchangeability)**：主要透過 **CAD 尺寸與空間干涉比對（Dimensional Analysis）** 來宣告非互換性防呆。臨床與工程實務上，只要圖紙證明幾何互斥，根本無須進行破壞性 50 N 實體強插試驗。
  - **-7 與 -20 的性能大考體系**：條文 6.1~6.6 涵蓋正壓 (330 kPa)、負壓 (88 kPa)、拉拔 (25N/35N)、自鬆 (0.02 N·m)、滑牙 (0.17 N·m) 與應力龜裂 (48h)。50 N 不屬於此體系。

### 矯正與預防措施 (CAPA)
1. **Slide 10 全面重構**：
   - 標題修正為：「大考之三：組裝後緊繃靜置 ≥ 48 小時耐應力龜裂，條文 5 三維幾何物理防呆互斥！」。
   - 右側卡片重構為「🛡️ 條文 5 非互換性幾何防呆（三維 CAD 空間互斥驗證）」，以微米級三維尺寸證明「天生互斥、不靠蠻力」。
   - 底部哲學橫條更新為時間軸（48h 應力）與空間軸（條文 5 三維互斥）之雙重守護。
   - 演講備忘錄 (`notes-data`) 與 `presentation-strategy-brief.md` 同步修正，全面清除 50 N 雜音。
2. **單檔同步**：
   - 執行 `npm run build:standalone`，重新編譯產出最新 `public/slides-standalone.html`（27.05 MB）。
3. **確效驗證**：
   - `npm test` 17/17 PASS（零功能迴歸）。
   - `npm run lint` 0 錯誤。
   - `npm run build` 打包通過。

---
## 版本：v8.32.0 生態系「玩家」角色與三者閉環納入投影片核心架構 (2026-09-05)

### 需求來源與目標
使用者指出投影片比喻中除了描述 -7 與 -20 的技術關係外，更需要把遵守這兩項規範的「玩家（Players/Stakeholders）」角色納入，並說清楚三者之間的動態協同與閉環關係。

### 根因分析與設計哲學
- *現象*：單純對比兩本書容易淪為抽象的概念對照；若無角色代入，聽眾無法感受「這兩本書在真實產業中是誰在讀、誰在用」。
- *第一性原理閉環*：
  1. **玩家 1：醫材製造廠商 (考生)** ➔ 翻開 **ISO 80369-7 專科考卷**，依圖紙雕刻微米公差與防呆幾何，鎖定及格線以達到性能驗收。
  2. **玩家 2：第三方公正檢驗所 (考官)** ➔ 翻開 **ISO 80369-20 考場 SOP**，架設 Annex C 不鏽鋼標準件，執行 27N/0.1N·m 上機裝配，出具客觀成績單。
  3. **玩家 3：官方監管機關 (FDA / TFDA，發照裁判)** ➔ 兩本合參，核對實驗室依 -20 出具的成績單是否達到 -7 要求的及格門檻，達標才核發醫療器材許可。
  4. **玩家 4：臨床護理師與病患 (最終安全享受者)** ➔ 在病房昏暗夜班盲插也插不進錯誤通道，病患翻身扯不脫、滴水不漏。

### 矯正與預防措施 (CAPA)
1. **Slide 1 升級**：
   - 左側對照卡融入「🏭 玩家角色：醫材製造廠 (考生)」與「🔬 玩家角色：第三方檢驗所 (考官)」。
   - 金色橫條升級為動態三者閉環：「**三者動態閉環：**廠商看 -7 造接頭 ➔ 檢驗所依 -20 測極限 ➔ 監管局 (FDA) 審查雙標準吻合度發照 ➔ 醫護病患安心盲插！」。
   - 更新演講備忘錄對應講稿。
2. **Slide 6 呼應**：
   - 舉例三正式對齊「玩家 1 (廠商考生) ➔ 玩家 2 (檢驗所考官) ➔ 玩家 3 & 4 (監管裁判與醫護病患)」。
3. **單檔同步**：
   - 執行 `npm run build:standalone`，產出最新 `public/slides-standalone.html`（27.05 MB）。
4. **確效驗證**：
   - `npm test` 17/17 PASS。
...
---
## 版本：v8.31.0 投影片認知清晰度重構：-7 與 -20 雙標準協同底層邏輯升級 (2026-09-05)

### 需求來源與目標
使用者指出 Slide 1 既有標籤（`📐 -7：告訴你管子長什麼樣` vs `🔬 -20：鐵面無私的及格大考`）無法讓人準確理解 ISO 80369-7 與 ISO 80369-20 之間的實質關聯與本質差異，甚至容易引發法規定義誤解。目標是引入第一性原理底層邏輯，將左側區域重構成直觀專業的「雙標準協同微型對照矩陣」。

### 根因分析 (RCA)
- *現象*：原標籤簡化過度，「告訴你管子長什麼樣」讓人誤以為 -7 僅是圖紙外觀；「鐵面無私的及格大考」更誤導讀者以為及格門檻由 -20 規定。
- *客觀法規事實 (SSOT)*：
  - **ISO 80369-7 是「專科醫院與考卷（What & Criteria）」**：專門管「血管/皮下（Intravascular / Hypodermic）」這一門，規定幾何尺寸、防呆要求、以及「必須通過哪些考試、及格分數是多少（條文 6.1~6.6 性能合格線）」。
  - **ISO 80369-20 是「國家級標準實驗室與考場規則（How to test）」**：全系列通用（-2 呼吸、-3 腸道、-6 神經、-7 血管統統適用）。它不給分數線，只提供統一的測試機具架設、前置裝配 SOP（附錄 B~J）。

### 矯正與預防措施 (CAPA)
1. **重構 Slide 1 左卡結構**：
   - 移除原先 3 個孤立的扁平標籤與次級條列。
   - 升級為獨立卡片「⚖️ 追問底層邏輯：國際標準委員會（ISO/TC 210）當初為什麼要把它們拆成兩本？」，內嵌左右對稱子卡：
     - **📘 ISO 80369-7**：專科考卷 (What)，專管血管/皮下，定幾何、防呆與及格分數線。
     - **🔬 ISO 80369-20**：共用考場 (How)，全系列通用，不給分數線，定治具機具與實驗 SOP。
   - 底部增設金色總結橫條：`🔗 本質真相：「-7 決定尺寸與驗收標準 ⟷ -20 提供共用實驗室量測手法」`。
2. **演講備忘錄同步**：
   - 更新 Slide 1 `notes-data` 講稿引導語。
3. **單檔編譯與建置**：
   - 執行 `npm run build:standalone`，將最新 SSOT 同步至 `public/slides-standalone.html`（27.05 MB）。
4. **確效驗證**：
   - `npm test` 17/17 PASS（零迴歸）。
   - `npm run lint` 通過。
   - `npm run build` 通過。
