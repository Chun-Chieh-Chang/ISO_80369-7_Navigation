...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-[var(--neo-bg)] text-[var(--neo-text)] font-sans selection:bg-blue-600 selection:text-white antialiased">
      {/* Header Bar */}
      ...
      {/* Footer */}
      <footer className="border-t border-[var(--neo-border)] bg-[var(--neo-surface)] py-6 text-center text-[13px] text-[var(--neo-muted)] mt-12 print:hidden"
              style={{ boxShadow: 'inset 0 1px 0 var(--neo-sl)' }}>
        <div className="max-w-[1920px] w-[96%] mx-auto px-4">
          <p className="font-semibold text-[var(--neo-text)] text-sm">
            {t.app.footer}
          </p>
          <p className="mt-1 text-[13px] text-[var(--neo-muted)]">
            Designed for Medical Device R&D, Quality Assurance, Regulatory Affairs (RA), and Testing Laboratory Engineers.
          </p>
          <p className="mt-2 text-[12px] text-[var(--neo-muted)]">
            Generated by <span className="font-medium text-[var(--neo-text)]">Wesley Chang</span>, QC Department @Mouldex, Sep-2026
          </p>
        </div>
      </footer>
